...
const BottomNavigation = ({ links }: BottomNavigationProps) => {
  return (
    <div>
      <div className='font-sans fixed bottom-0 left-0 right-0 flex max-h-20 w-full justify-around border-t border-t-cgreen-7 bg-cgreen-6 p-2 z-50'>
        {links.map((link) => {
...
const BottomNavigationItem = ({ data }: BottomNavigationItemProps) => {
  ...
  const isActive = newPath === data.href ? 'bg-cgreen-4 text-white' : 'text-neutral-400'
